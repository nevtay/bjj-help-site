...
const SelectedPosition = () => {
    ...
    let { name, description, imagesUrl } = selectedPositionData

    let images = <span className="mt-10 w-5/6 flex flex-row flex-wrap justify-start">
        {imagesUrl && imagesUrl.map(img => {
            return (
    ...
    return (
        <div>
            ...
            <h2 className='w-4/6 text-4xl leading-normal text-yellow-300 mb-5'>{name}</h2>
            <p className='w-5/6 md:w-4/6 text-small leading-normal text-yellow-300 mb-5'>{description}</p>
            {images}
        </div>
    )
}
